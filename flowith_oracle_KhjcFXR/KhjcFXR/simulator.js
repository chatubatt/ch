export class Simulator {
    ...
    updateUI() {
        ...
        const kpis = [
            ...
            { id: 'abandonment', name: 'Taxa de Abandono', value: `${this.state.kpis.abandonment.toFixed(1)}%`, target: this.state.goals.abandonment, dir: 'down' },
            { id: 'queue', name: 'Chamadas em Fila', value: this.state.callQueue.length, target: 10, dir: 'down' },
            { id: 'maxWait', name: 'Maior Espera', value: `${Math.round(this.state.kpis.maxWait * 60)}s`, target: this.state.goals.maxWait, dir: 'down' },
        ];
        kpiDashboard.innerHTML = kpis.map(kpi => {
            let statusClass = 'sim-kpi-green';
            const val = parseFloat(kpi.value);
            if((kpi.dir === 'up' && val < kpi.target * 0.95) || (kpi.dir === 'down' && val > kpi.target * 1.05)) statusClass = 'sim-kpi-yellow';
            if((kpi.dir === 'up' && val < kpi.target * 0.85) || (kpi.dir === 'down' && val > kpi.target * 1.25)) statusClass = 'sim-kpi-red';
            return `
                <div class="sim-kpi-card p-4 rounded-lg ${statusClass}">
                    <div class="flex justify-between items-start">
                        <div class="font-semibold text-gray-700">${kpi.name}</div>
                        <div class="text-xs text-gray-500">Meta: ${kpi.target}${kpi.id.includes('abandonment') || kpi.id.includes('sla') ? '%' : ''}</div>
                    </div>
                    <div class="text-3xl font-bold text-gray-900 mt-1">${kpi.value}</div>
                </div>`;
        }).join('');
        

        const agentPanel = this.container.querySelector('#agent-status-panel');
        agentPanel.innerHTML = `
            <div class="flex justify-between items-center p-3 bg-white rounded-md border"><div><i data-lucide="phone" class="inline-block mr-2 text-red-500"></i>Em Atendimento</div><div class="font-bold text-lg">${this.state.agentStatus.on_call}</div></div>
            <div class="flex justify-between items-center p-3 bg-white rounded-md border"><div><i data-lucide="user-check" class="inline-block mr-2 text-green-500"></i>Disponíveis</div><div class="font-bold text-lg">${this.state.agentStatus.available}</div></div>
            <div class="flex justify-between items-center p-3 bg-white rounded-md border"><div><i data-lucide="briefcase" class="inline-block mr-2 text-yellow-500"></i>Back-office</div><div class="font-bold text-lg">${this.state.agentStatus.back_office}</div></div>
            <div class="flex justify-between items-center p-3 bg-gray-200 rounded-md border"><div><i data-lucide="users" class="inline-block mr-2 text-gray-600"></i>Total Logado</div><div class="font-bold text-lg">${this.state.agentStatus.on_call + this.state.agentStatus.available}</div></div>
        `;
        lucide.createIcons();
    }
}
